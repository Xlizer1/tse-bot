...
// Migrate targets and contributions to database
    ...
    if (targetsData.resources && typeof targetsData.resources === 'object') {
      for (const [key, resource] of Object.entries(targetsData.resources)) {
        ...
        const [result] = await pool.execute(
          ...
          [action, resourceValue, resource.target, resource.createdBy || 'unknown', resource.createdAt || new Date().toISOString()]
        );
        ...
        if (progress.contributions && Array.isArray(progress.contributions)) {
          ...
          console.log(`Migrated ${progress.contributions.length} contributions for ${action} ${resourceValue}`);
        }
      }
      
      console.log(`Migrated ${Object.keys(targetsData.resources).length} targets`);
    }
