...
async function checkAndSendReport(client) {
    try {
        ...
        if (currentTime === time && currentDate !== lastReportDate) {
            ...
            console.log(`Auto report sent and last report date updated to ${currentDate}`);
        }
    } catch (error) {
...
async function sendDailyReport(channel) {
    try {
        ...
        try {
            await channel.send('An error occurred while generating the daily report.');
        } catch (sendError) {
