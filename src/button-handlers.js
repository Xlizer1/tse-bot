...
async function handleSelectMenuInteraction(interaction) {
    ...
    if (interaction.customId === 'resource_select') {
        try {
            ...
            const resourceCategory = target.action === 'mine' ? 'mining' : 
                                   target.action === 'salvage' ? 'salvage' : 'haul';
            
            const resource = await ResourceModel.getByValueAndType(target.resource, resourceCategory);
            ...
            const modal = new ModalBuilder()
                ...
                .setTitle(`Log ${target.action.charAt(0).toUpperCase() + target.action.slice(1)} ${resourceName}`);
...
async function handleModalSubmitInteraction(interaction) {
    ...
    if (interaction.customId.startsWith('add_modal_')) {
        try {
            ...
            await interaction.reply({
                content: `Logged: You ${target.action}d ${amount} SCU of ${resourceName} at ${location}\n` +
