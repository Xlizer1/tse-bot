...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('clear')
                .setDescription('Clear the debug log')),
    
    async execute(interaction) {
        // Check admin permissions
        if (!interaction.member.permissions.has('Administrator')) {
            return interaction.reply({ 
                content: 'You do not have permission to use this command. Only administrators can access debug tools.', 
                ephemeral: true 
            });
        }
        ...
        if (subcommand === 'log') {
            ...
            const truncatedLog = logContent.length > 4000 
                ? '...' + logContent.slice(-3997) 
                : logContent;
            ...
            const debugEmbed = new EmbedBuilder()
                ...
                .setDescription('```\n' + truncatedLog + '\n```')
                .setFooter({ text: `Showing last ${lines} lines (truncated to 4000 chars if needed)` })
                .setTimestamp();
