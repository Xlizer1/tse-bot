    ...
    async execute(interaction) {
        ...
        if (!interaction.member.permissions.has('Administrator')) {
            return interaction.reply({ 
                content: 'You do not have permission to use this command. Only administrators can export data.', 
                ephemeral: true 
            });
        }
        ...
        try {
            ...
            // Add headers
            summarySheet.addRow([]);
            const summaryHeaders = summarySheet.addRow(['Resource', 'Action', 'Target (SCU)', 'Current (SCU)', 'Progress']);
            ...
            // Add headers
            detailsSheet.addRow([]);
            const detailsHeaders = detailsSheet.addRow(['Timestamp', 'User', 'Resource', 'Action', 'Amount (SCU)', 'Location']);
            ...
            const userHeaders = userSummarySheet.addRow(['User', 'Total Contributions (SCU)', 'Percentage of Total']);
            ...
            allContributions.forEach(contribution => {
                ...
                userTotals.set(contribution.username, userTotals.get(contribution.username) + contribution.amount);
            ...
            const tempFile = path.join(__dirname, '..', 'data', 'resource_report.xlsx');
            await workbook.xlsx.writeFile(tempFile);
            
            // Create attachment
            const attachment = new AttachmentBuilder(tempFile, { name: 'resource_report.xlsx' });
            ...
            await interaction.editReply('There was an error exporting the data.');
        }
    },
};