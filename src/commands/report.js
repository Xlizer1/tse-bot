...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('report')
        .setDescription('Report your resource collection (mining, salvaging, or hauling)')
    ...
    async execute(interaction) {
        try {
            ...
            const percentage = Math.floor((current / updatedTarget.target_amount) * 100);
            
            // Get resource name for display
            const resourceCategory = action === 'mine' ? 'mining' : 
                                   action === 'salvage' ? 'salvage' : 'haul';
            
            const resourceInfo = await ResourceModel.getByValueAndType(resource, resourceCategory);
            const resourceName = resourceInfo ? resourceInfo.name : 
                               (resource.charAt(0).toUpperCase() + resource.slice(1));
            
            // Reply with acknowledgment and progress
            await interaction.reply(
                `Reported: ${interaction.user.username} ${action}d ${amount} SCU of ${resourceName} at ${location}\n` +
            ...
            await interaction.reply({ 
                content: 'An error occurred while reporting your contribution.', 
                ephemeral: true 
            });
        }
    }
